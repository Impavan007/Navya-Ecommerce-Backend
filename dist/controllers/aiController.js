...
exports.handleTtsProxy = exports.handleStylistChat = void 0;
const axios_1 = __importDefault(require("axios"));
const ollama_1 = require("../config/ollama");
...
const handleStylistChat = async (req, res) => {
    try {
        ...
        if (products.length === 0) {
            ...
            products = await Product_1.default.find({ status: 'Active' })
                .select('name brand price description category')
                .populate('category', 'name')
                .limit(10);
        }
...
4. If no products in the catalog fit the description, suggest styling tips generally but explain that we do not have an exact matching item in our catalog right now.`;
        ...
        const headers = {};
        if (ollama_1.OLLAMA_CONFIG.apiKey) {
            headers['Authorization'] = `Bearer ${ollama_1.OLLAMA_CONFIG.apiKey}`;
        }
        const cleanBaseUrl = ollama_1.OLLAMA_CONFIG.baseUrl.replace(/\/$/, "");
        const targetUrl = cleanBaseUrl.endsWith("/api")
            ? `${cleanBaseUrl}/chat`
            : `${cleanBaseUrl}/api/chat`;
        // Query Ollama Chat API with streaming enabled
        const ollamaResponse = await axios_1.default.post(targetUrl, {
            model: ollama_1.OLLAMA_CONFIG.model,
            messages: messagesToSend,
            options: {
                temperature: ollama_1.OLLAMA_CONFIG.temperature,
                num_ctx: ollama_1.OLLAMA_CONFIG.numCtx
            },
            stream: true
        }, {
            headers,
...
exports.handleStylistChat = handleStylistChat;
// Free Google Translate Text-to-Speech proxy service (CORS-safe, key-free)
const handleTtsProxy = async (req, res) => {
    try {
...
exports.handleTtsProxy = handleTtsProxy;
